...
export default class FaceSvg extends React.PureComponent {
  state = {
    path: PATHS.neutral,
  ...
  componentDidUpdate(prevProps) {
    const { faceStatus } = this.props;

    if (this.props.faceStatus !== prevProps.faceStatus) {
      faceStatus &&
        this.interpolatePaths(faceStatus ? "smile" : "sad", faceStatus ? 2 : 0);
    }
  }
